 ...
 */
esp_err_t _wifi_clear_default_wifi_handlers(void)
{
 ...
 */
esp_err_t _wifi_set_default_wifi_handlers(void)
{
...
fail:
    _wifi_clear_default_wifi_handlers();
    return err;
}
 ...
 */
esp_err_t wifi_set_default_wifi_sta_handlers(void)
{
    return _wifi_set_default_wifi_handlers();
}

/**
 * @brief Set default handlers for AP (official API)
 */
esp_err_t wifi_set_default_wifi_ap_handlers(void)
{
    return _wifi_set_default_wifi_handlers();
}
 ...
 */
esp_err_t wifi_clear_default_wifi_driver_and_handlers(void *esp_netif)
{
    int i;
    ...
    if (i == MAX_WIFI_IFS)
    { // if all wifi default netifs are null
...
esp_err_t netif_attach_wifi_station(esp_netif_t *esp_netif)
{
    if (esp_netif == NULL)
    {
...
esp_err_t netif_attach_wifi_ap(esp_netif_t *esp_netif)
{
    if (esp_netif == NULL)
    {
        return ESP_ERR_INVALID_ARG;
 ...
 */
esp_netif_t *netif_create_default_wifi_ap(void)
{
    esp_netif_config_t cfg = ESP_NETIF_DEFAULT_WIFI_AP();
    esp_netif_t *netif = esp_netif_new(&cfg);
    assert(netif);
    netif_attach_wifi_ap(netif);
    wifi_set_default_wifi_ap_handlers();
    return netif;
}
 ...
 */
esp_netif_t *netif_create_default_wifi_sta(void)
{
    esp_netif_config_t cfg = ESP_NETIF_DEFAULT_WIFI_STA();
    esp_netif_t *netif = esp_netif_new(&cfg);
    assert(netif);
    netif_attach_wifi_station(netif);
    wifi_set_default_wifi_sta_handlers();
    return netif;
}
 ...
 */
esp_err_t netif_create_default_wifi_mesh_netifs(esp_netif_t **p_netif_sta, esp_netif_t **p_netif_ap)
{
    ...
    assert(netif_ap);
    ESP_ERROR_CHECK(netif_attach_wifi_ap(netif_ap));
    ESP_ERROR_CHECK(wifi_set_default_wifi_ap_handlers());
    ...
    assert(netif_sta);
    ESP_ERROR_CHECK(netif_attach_wifi_station(netif_sta));
    ESP_ERROR_CHECK(wifi_set_default_wifi_sta_handlers());
